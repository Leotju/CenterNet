...
def ctdet_post_process_ms(dets, dets_s8, dets_s16, c, s, h, w, h_s8, w_s8, h_s16, w_s16, num_classes):
    # dets: batch x max_dets x dim
    # return 1-based class det dict
    ret = []
    for i in range(dets.shape[0]):
        ...
        dets[i, :, 2:4] = transform_preds(
            dets[i, :, 2:4], c[i], s[i], (w, h))
        classes = dets[i, :, -1]
        dets_s8[i, :, :2] = transform_preds(
            dets_s8[i, :, 0:2], c[i], s[i], (w_s8, h_s8))
        dets_s8[i, :, 2:4] = transform_preds(
            dets_s8[i, :, 2:4], c[i], s[i], (w_s8, h_s8))
        classes_s8 = dets_s8[i, :, -1]
        dets_s16[i, :, :2] = transform_preds(
            dets_s16[i, :, 0:2], c[i], s[i], (w_s16, h_s16))
        dets_s16[i, :, 2:4] = transform_preds(
            dets_s16[i, :, 2:4], c[i], s[i], (w_s16, h_s16))
        classes_s16 = dets_s16[i, :, -1]
        for j in range(num_classes):
            inds = (classes == j)
            top_preds[j + 1] = np.concatenate([
                dets[i, inds, :4].astype(np.float32),
                dets[i, inds, 4:5].astype(np.float32)], axis=1).tolist()
            inds_s8 = (classes_s8 == j)
            top_preds_s8[j + 1] = np.concatenate([
                dets_s8[i, inds_s8, :4].astype(np.float32),
                dets_s8[i, inds_s8, 4:5].astype(np.float32)], axis=1).tolist()
            inds_s16 = (classes_s16 == j)
            top_preds_s16[j + 1] = np.concatenate([
                dets_s16[i, inds_s16, :4].astype(np.float32),
                dets_s16[i, inds_s16, 4:5].astype(np.float32)], axis=1).tolist()
            top_preds_total[j + 1] = top_preds[j + 1] + top_preds_s8[j + 1] + top_preds_s16[j + 1]
            # top_preds_total[j+1] = top_preds_s8[j + 1]
        ret.append(top_preds_total)
    return ret
