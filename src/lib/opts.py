...
class opts(object):
    ...
    def update_dataset_info_and_set_heads(self, opt, dataset):
        ...
        if opt.task == 'exdet':
            ...
            if opt.reg_offset:
                opt.heads.update({'reg': 2})
        elif opt.task == 'ctdet_ms':
            # assert opt.dataset in ['pascal', 'coco']
            opt.heads = {
                'hm_s': opt.num_classes,
                'wh_s': 2 if not opt.cat_spec_wh else 2 * opt.num_classes,
                'hm_m': opt.num_classes,
                'wh_m': 2 if not opt.cat_spec_wh else 2 * opt.num_classes,
                'hm_l': opt.num_classes,
                'wh_l': 2 if not opt.cat_spec_wh else 2 * opt.num_classes,
            }
            if opt.reg_offset:
                opt.heads.update({
                    'reg_s': 2,
