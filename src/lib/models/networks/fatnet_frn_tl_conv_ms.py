...
class PosePangNet(nn.Module):
    ...
    def __init__(self, heads, head_conv, **kwargs):
        ...
        self.trans_small_conv = BasicConv(32, 64, kernel_size=1, dilation=1, padding=0)
        self.trans_mid_conv = BasicConv(64, 64, kernel_size=1, dilation=1, padding=0)
        self.trans_large_conv = BasicConv(128, 64, kernel_size=1, dilation=1, padding=0)

        self.small_convs = BasicConv(64, 64, kernel_size=5, dilation=2, padding=4)
        self.mid_convs = BasicConv(64, 64, kernel_size=9, dilation=2, padding=8)
        self.large_convs = BasicConv(64, 64, kernel_size=13, dilation=2, padding=12)
    ...
    def forward(self, x):
        ...
        small_feats = self.small_convs(self.trans_small_conv(output[0]))
        mid_feats = self.mid_convs(self.trans_mid_conv(output[1]))
        large_feats = self.large_convs(self.trans_large_conv(output[2]))
        ...
        for head in self.heads:
            if '_s' in head:
                ret[head] = self.__getattr__(head)(small_feats)
            if '_m' in head:
                ret[head] = self.__getattr__(head)(mid_feats)
            if '_l' in head:
                ret[head] = self.__getattr__(head)(large_feats)
        return [ret]
