...
class Pang_unit_stride_mb(nn.Module):  #### basic unit
    def __init__(self, cin, cout, bn):
        super(Pang_unit_stride_mb, self).__init__()
        # if bn==True:
        #     bias = False
        # else:
        #     bias = True
        ...
        self.branch8 = BasicConv(cin, cout, kernel_size=3, stride=1, padding=1, bn=bn, bias=bias)
        self.branch4 = BasicConv(cin, cout, kernel_size=3, stride=1, padding=1, bn=bn, bias=bias)
        self.branch2 = BasicConv(cin, cout, kernel_size=3, stride=1, padding=1, bn=bn, bias=bias)
        self.branch1 = BasicConv(cin, cout, kernel_size=3, stride=1, padding=1, bn=bn, bias=bias)

        self.branch0 = BasicConv(cin, cout, kernel_size=1, stride=1, padding=0, bn=bn, bias=bias)
        self.cin = cin
        self.cout = cout

    def forward(self, x):
        x8 = F.max_pool2d(x, kernel_size=8, stride=8)
        x8 = self.branch8(x8)
        x8 = F.upsample_nearest(x8, scale_factor=8)

        x4 = F.max_pool2d(x, kernel_size=4, stride=4)
        x4 = self.branch4(x4)
        x4 = F.upsample_nearest(x4, scale_factor=4)

        x2 = F.max_pool2d(x, kernel_size=2, stride=2)
        x2 = self.branch8(x2)
        x2 = F.upsample_nearest(x2, scale_factor=2)
    ...
    def _make_layers_pangnet(self, batch_norm=True):
        ...
        for ic, v in enumerate(cfg):
            ...
            if ic <= 1:
                ...
                layers.append(Pang_unit_stride_mb(in_channels, v, bn=batch_norm))
