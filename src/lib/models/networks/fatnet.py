...
class Pang_unit_stride_mb(nn.Module):  #### basic unit
    def __init__(self, cin, cout, bn):
        ...
        self.branch32 = nn.Sequential(
            BasicConv(cin, cin // groups, 1, 1, 0),
            BasicConv(cin // groups, cin // groups, kernel_size=3, stride=2, padding=1, bn=bn, bias=bias),
            BasicConv(cin // groups, cout, 1, 1, 0))
        self.branch31 = nn.Sequential(
            BasicConv(cin, cin // groups, 1, 1, 0),
            BasicConv(cin // groups, cin // groups, kernel_size=3, stride=1, padding=1, bn=bn, bias=bias),
            BasicConv(cin // groups, cout, 1, 1, 0))
        self.branch1 = BasicConv(cin, cout, kernel_size=1, stride=1, padding=0, bn=bn, bias=bias)
        self.cin = cin
        self.cout = cout
    ...
    def forward(self, x):
        x98 = self.branch98(x)
        x98 = F.upsample_nearest(x98, scale_factor=8)

        x54 = self.branch54(x)
        x54 = F.upsample_nearest(x54, scale_factor=4)

        x32 = self.branch32(x)
        x32 = F.upsample_nearest(x32, scale_factor=2)

        x31 = self.branch31(x)

        x1 = self.branch1(x)
        if self.cin == self.cout:
            x0 = x1 + x32 + x54 + x98 + x31 + x
        else:
            x0 = x1 + x32 + x54 + x98 + x31
        return x0
    ...
    def __init__(self, heads, head_conv, **kwargs):
        ...
        for head in sorted(self.heads):
            ...
            if head_conv > 0:
                ...
                fc = nn.Sequential(
                    BasicConv(128, head_conv, kernel_size=3, padding=1, bias=True, bn=True, relu=True),
