...
class Pang_unit_stride_d2(nn.Module):  #### basic unit
    def __init__(self, cin, cout, bn):
        ...
        self.branch0 = BasicConv(cin, cout, kernel_size=3, stride=2, padding=2, dilation=2, bn=bn, bias=bias)
        self.branch1 = BasicConv(cin, cout, kernel_size=1, stride=1, padding=0, bn=bn, bias=bias)
        self.cin = cin
        self.cout = cout
    ...
    def _make_layers_pangnet(self, batch_norm=True):
        ...
        for ic, v in enumerate(cfg):
            ...
            if ic <= 1:
                ...
                layers.append(Pang_unit_stride_d2(in_channels, v, bn=batch_norm))
            in_channels = v
        return layers
